...
export const createPresignedPostDataPromise = (
  ...
  const s3Endpoint = new URL(AwsConfig.endPoint)
  const isR2 = s3Endpoint.hostname.endsWith('.r2.cloudflarestorage.com')
  if (isR2) {
    AwsConfig.s3.config.signatureVersion = 'v4'
  }
    ...
    isR2
      ? AwsConfig.s3
          ...
          .then((url) => ({
            ...
            fields: {
              key,
            } as unknown as PresignedPost['fields'],
          }))
